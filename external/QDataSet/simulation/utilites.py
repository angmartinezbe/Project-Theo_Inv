...
def simulate(sim_parameters):
    ...
    for idx_batch in range(sim_parameters["num_ex"] // sim_parameters["batch_size"]):
        ...
        for idx_ex in range(sim_parameters["batch_size"]):
            Results = {"sim_parameters"  : sim_parameters,
                       ...
                       "distorted_pulses": pulses[idx_ex:idx_ex+1, :],  # sin distorsión, igual a pulses
            ...
            # open a pickle file
            fname = f"{sim_parameters['name']}_ex_{idx_ex + idx_batch*sim_parameters['batch_size']}.pkl"
            f = open(fname, 'wb')
            # save the results
    ...
    # close the zip file
    fzip.close()
    # portable copy of the generated zip (works on Windows/Linux/macOS)
    dst_dir = Path.cwd() / "datasets"
    dst_dir.mkdir(parents=True, exist_ok=True)
    src = Path(f"{sim_parameters['name']}.zip")
    shutil.copyfile(src, dst_dir / src.name)
    cluster_dir = Path("/projects/QuantumDS")
    if cluster_dir.is_dir():
        shutil.copyfile(src, cluster_dir / src.name)
    ###########################################################################
    ...
    for idx_batch in range(sim_parameters["num_ex"] // sim_parameters["batch_size"]):
        ...
        for idx_ex in range(sim_parameters["batch_size"]):
            ...
            # open a pickle file
            fname = f"{sim_parameters['name']}_D_ex_{idx_ex + idx_batch*sim_parameters['batch_size']}.pkl"
            f = open(fname, 'wb')
            # save the results
            pickle.dump(Results, f, -1)
            # close the pickle file
    ...
    # close the zip file
    fzip.close() 
    # portable copy of the generated distorted zip
    dst_dir = Path.cwd() / "datasets"
    dst_dir.mkdir(parents=True, exist_ok=True)
    src = Path(f"{sim_parameters['name']}_D.zip")
    shutil.copyfile(src, dst_dir / src.name)
    cluster_dir = Path("/projects/QuantumDS")
    if cluster_dir.is_dir():
        shutil.copyfile(src, cluster_dir / src.name)
